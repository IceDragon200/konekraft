...
module Sadie
  module SASM
    module Constants
      ...
      ## registers
      REG_ACCUMULATOR = REG_A = 0x0       #  8-bit Accumulator
      REG_FLAG = REG_F = 0x1              #  8-bit Flag Register
      REG_AF       = (REG_A << 4) | REG_F # 16-bit pair Register AF - Special
      REG_B        = 0x2                  #  8-bit general Register B
      REG_C        = 0x3                  #  8-bit general Register C
      REG_BC       = (REG_B << 4) | REG_C # 16-bit pair Register BC - General
      REG_D        = 0x4                  #  8-bit general Register D
      REG_E        = 0x5                  #  8-bit general Register E
      REG_DE       = (REG_D << 4) | REG_E # 16-bit pair Register DE - General
      REG_H        = 0x6                  #  8-bit general Register H
      REG_L        = 0x7                  #  8-bit general Register L
      REG_HL       = (REG_H << 4) | REG_L # 16-bit pair Register HL - Memory
      REG_PSW      = 0x8                  # Process State Word
      REG_STACK_POINTER = REG_SP = 0x9    # 16-bit Stack Pointer (SP)
      REG_PROGRAM_COUNTER = REG_PC = 0xA  # 16-bit Program Counter (PC)
      REG_MEMORY = REG_M = 0xB # Not really a register used to access the memory bank
