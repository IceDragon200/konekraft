#
# konekraft/sasm0/assembler.rb
#
require 'konekraft/sasm0/ast'
require 'konekraft/sasm0/program'
    ...
    class Assembler
      ...
      def self.make_instructions(data)
        data.map do |code, params|
          ...
          #   changed
          Program::Instruction.new(Konekraft::Slate::CPU, code, params)
        end
      end
      ...
      def self.assemble_string(str)
        lxs = str.each_line.map { |line| Konekraft::SlateAssembly::Lexer.lex(line) }
        asts = lxs.map { |lx| Konekraft::SlateAssembly::Parser.parse(lx, accept: :all) }
        Konekraft::SlateAssembly::Assembler.assemble(asts.flatten)
      end
