...
require 'konekraft/slate1/memory'
require 'konekraft/slate1/cpu/clock'
require 'konekraft/slate1/cpu/register'
require 'konekraft/slate1/cpu/register_pair'
require 'konekraft/slate1/cpu/ports'
require 'konekraft/slate1/util/cpu_tool'
...
module Konekraft
  module Slate1
    class CPU
      include SlateAssembly0::Constants
      ...
      def init_register
        ...
        REGISTERS.each do |code|
          ...
          @register[code] = CPU::Register.new(self, size)
        end.freeze
        REGISTER_PAIR.each_pair do |code, (h_id, l_id)|
          h, l = @register[h_id], @register[l_id]
          @register[code] = CPU::RegisterPair.new(self, h, l)
        end
      ...
      # init_ports
      def init_ports
        @ports = CPU::Ports.new(self, 256, 256)
      end
      ...
      def self.def_inst(code, symbol, *params, &rel_func)
        ...
        embed_param_types = embed_params.map { |o| CpuTool.identify_type(o) }
        param_types = embed_param_types + (params[lst, hardsize] || [])
        inst_spec = SlateAssembly0::InstructionSpec.new(code, symbol,
      ...
      def find_and_set_flags(result)
        ...
        self.flag_p = (BitTool.count_one_bits(result) % 2) != 0 ? 1 : 0
      end
