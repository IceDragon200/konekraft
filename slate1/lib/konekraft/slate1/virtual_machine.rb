...
module Konekraft
  module Slate1
    class VirtualMachine
      ...
      # init_memory
      def init_memory
        @memory = Memory.new(@cpu, @memory_size, @memory_blocksize)
      end
      ...
      def exec_eval(str)
        SlateAssembly0::Assembler.assemble_string(str).each do |inst|
          puts inst # DEBUG
          @slate.exec(inst)
        end
      end
      ...
      def exec(*args)
        ...
        when SlateAssembly0::Program::Instruction then exec_inst(arg)
        else
          raise ArgumentError,
